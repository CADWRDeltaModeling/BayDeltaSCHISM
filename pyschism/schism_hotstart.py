...
class hotstart(object):
    ...
    """
    def __init__(self,yaml_fn,modules=['TEM','SAL'],**kwargs): # will change these into yaml file on
    ...
    def read_yaml(self):
        ...
        rfl = ['hgrid_input_file', 'vgrid_input_file','date'] # remove these items from the list. 
    ...
    def create_hotstart(self):
        ...
        self.wet_dry_check() # correct wet/dry cells depending on water level (eta2 value):mostly not needed. 
        self.map_to_schism()        
        ...
        if 'SED' in variable:
            if 'Nbed' not in self.__dict__.keys():                
                if os.path.isfile("sediment.in"):
                    ...
                    raise FileNotFoundError("sediment.in is required if SED module is turned on")              
            kwargs_options.update({'Nbed':self.Nbed})    
        if kwargs_options:        
            var = VariableField(v_meta,variable, self.mesh, self.depths, self.date, **kwargs_options)
        else:
            var = VariableField(v_meta,variable, self.mesh, self.depths, self.date)
...
#            z = self.mesh.build_z()
        ...
        if any(dry_nodes):
            ...
            idry_s = np.array([idry[n].mean(axis=0) for n in self.mesh.edges[:,:2]]) # node to side
            idry_s[idry_s!=0] = 1 # if there is one dry node, the element is dry? 
            idry_e = np.array([idry[list(n)].mean(axis=0) for n in self.mesh.elems]) #node to elem
            idry_e[idry_e!=0] = 1 # if there is one dry node, the side is dry? 
    ...
    def map_to_schism(self):
        ...
        ntracers, ntrs, irange_tr, tr_mname = n_tracers(param_in,modules=self.modules)
        ...
        for key,var_values in zip(schism_hotstart_var.keys(),schism_hotstart_var.values()):
            if isinstance(var_values,str):
                ...
                var_temp = np.asarray([nc_dataset[var_values][v].values for v in var_values])
...
class VariableField(object):
    ...
    def get_grid(self): 
        ...
        """
        options = ['node3D','node2D','edge','elem', 'prism','bed','bedfrac']
    ...
    def initializer_options(self):
        ...
        options = ['text_init','simple_trend','patch_init','extrude_casts','obs_points']
    ...
    def GenerateField(self):
        ...
        if np.any(np.isnan(var)):
            raise ValueError("%s field has nan in it: check input data and variable name"%self.variable_name)
    ...
    def simple_trend(self,ini_meta=None,inpoly=None):  
        ...
        if isinstance(value,(float,int)):
            v_ini = np.zeros((n_hgrid,self.n_vgrid)) # initialize the 3D field
            return v_ini + value
            ...
            if self.variable_name in ['SED3D_bed','SED3D_bedfrac']:
                ...
                vmap = eval(value) # x and y based function where x and y are lat and lon.
        ...
        if text_fn.endswith('.ic'): 
            if self.centering == 'node':
                ...
                if icmesh.n_nodes() == self.n_nodes:
                    ...
                    raise Exception("The node of %s is incompatible with the grid node in hgrid.gr3"%text_fn)                   
                
                if len(vmap) != self.n_hgrid: # if the required input is on element, but values on nodes are provided. 
                    vmap = self.node2elem_values(vmap)
            else:
                raise ValueError('%s gives value on node, but value on %s is required'%(text_fn, self.centering))
            ...
            if self.centering in ['elem', 'prism']:
                ...
                if len(vmap) != self.n_elems:
                    raise("The element of %s is incompatible with grid element in hgrid.gr3"%text_fn)
                if len(vmap) != self.n_hgrid: # if the required input is on nodes, but values on elements are provided 
                    vmap = self.elem2node_values(vmap)
            else:
                raise ValueError('%s gives value on element, but value on %s is required'%(text_fn, self.centering))
            ...
            if nh !=self.n_hgrid:
                if self.n_hgrid == self.mesh.n_nodes:
                    ...
                    raise ValueError("The horizontal dim of %f does not match with the mesh grid"%text_fn)
        else: 
            raise TypeError("Only .ic, .prop, or .nc are allowed as text input")
    ...
    def patch_init(self,poly_fn=None):# regional based initializer 
        try:
            ...
            if not poly_fn:
                raise EOFError("regional polygon file is needed for pathch_ini implementation")            
        if poly_fn.endswith('shp'):
            mapping = geo_tools.Contiguity_Check(self.mesh,poly_fn) # perform contiguity check and return a mapping array if successful. 
        else: 
        ...
        for i, r in enumerate(self.ini_meta['regions']):
            ...
            if np.any(np.isnan(v)):
                raise ValueError("region %f has nan in %s field"%(r,self.variable_name))
            v_merge[inpoly,:] = v 
            print(i,r)
        return v_merge        
    ...
    def extrude_casts(self,ini_meta=None,inpoly=None): # USGS cast (2D observational points)
        ...
        if 'Station' in polaris_data.columns:
            polaris_cast = polaris_data[polaris_data.datetime == cast_date][['Station',variable,'Depth']]
        else:
        ...
        grid_depths = self.vgrid[inpoly,:]*-1.0 # the depths computed are negative but polaris depths are positive               
        for s in polaris_cast.index.unique():
...
#        return v            
    ...
    def create_dataarray(self,var):  
        if self.centering == 'prism': # for prism (tracer variables), tr_el and tr_nd0 need to be calculated. 
        # convert from node to element values for tracers
            var_temp = self.node2elem_values(var) # first average to the node center horizontally
            var_temp = np.asarray(var_temp)
            var_el = np.zeros((self.n_elems,self.n_vert_levels))
            var_el[:,1:] = (var_temp[:,:-1] + var_temp[:,1:])/2.0 # then average to the element center vertically 
...
def read_param_in(infile):
    ...
    with open(infile,'r') as f:
        for line in f:
            line = line.rstrip('\n')
            line = line.strip().split('!')[0] # only take the portion of the arguement before the comment
...
!    11: FABM    
    ...
    if "SED" in modules and 'sed_class' in param.keys():
        irange_tr_1.append(sum(ntrs)) 
        ntrs.append(param['sed_class']) # sediment concentration for different classes (g/m3??)
        for t in range(ntrs[-1]):
            tr_mname.append('SED_%d'%(t+1))
        irange_tr_2.append(sum(ntrs))
    
    if "ECO" in modules and 'eco_class' in param.keys():
        raise NotImplementedError("Initiating ECO from hotstart.nc may not have been implemented")
        irange_tr_1.append(sum(ntrs)) 
    ...
    if "COSINE" in modules:
        irange_tr_1.append(sum(ntrs)) 
        ntrs.append(13)
        COS_name = ['NO3','SiO4','NH4','S1','S2','Z1','Z2','DN','DSi','PO4','DOX','CO2','ALK']
...
def hotstart_to_outputnc(hotstart_fn,init_date,hgrid_fn = 'hgrid.gr3',vgrid_fn = 'vgrid.in',outname="hotstart_out.nc"):
    ...
    for c in ['edge','face','node']:
        ...
        gx = xr.DataArray(utm_data[0],dims=hgrid_nc['SCHISM_hgrid_%s_x'%c].dims)
        gx.attrs = hgrid_nc['SCHISM_hgrid_%s_x'%c].attrs
        gx.attrs['standard_name'] = 'projection_x_coordinate'
        gx.attrs['units'] = 'm'
    
        gy = xr.DataArray(utm_data[1],dims=hgrid_nc['SCHISM_hgrid_%s_y'%c].dims)
    ...
    if 'time' in list(hnc.variables): # if 'time' variable already in hnc, drop it. 
        hnc = hnc.drop('time')
        
    # add zcor and time dimension    
    zcor = hnc.z.values[np.newaxis,:] # zcor is positive upwards: all wet node values are negative. 
    hgrid_nc['zcor'] = xr.DataArray(zcor, dims=['time','nSCHISM_hgrid_node','nSCHISM_vgrid_layers'])
...
#   the i23d value has to be accurate for VisIt to plot.  
    for v in hvar:
        ...
        if dl <3: #1D or 2D array, just expand.
            ...
            vn = v
            hnc[vn] = hnc[vn].squeeze().expand_dims({'time':time}).astype(float)
            ...
            if dl==1:
                hnc[vn].attrs['data_vertical_center'] = "full" # 2D are all on full levels. 
            ...
            if c3 == 'MBEDP':
                ...
                for i,n in enumerate(sl):
                    vn = "%s_%s"%(v,n)
                    hnc[vn] = hnc[v].isel(MBEDP=i).squeeze().expand_dims('time').astype(float)
                    hnc[vn].attrs['mesh'] = "SCHISM_hgrid"
                    dhc = hnc[vn].dims[1].split('_')[-1]
                    if dhc == 'face':
                        dhc = 'elem'
                    hnc[vn].attrs['data_horizontal_center'] = dhc
                    hnc[vn].attrs['data_vertical_center'] = "full" # so far, everything should be on full
                    hnc[vn].attrs['i23d'] = 4
                    hnc[vn].attrs['ivs'] = hgrid_nc.zcor.ivs
            else:
                hnc[vn].attrs['data_vertical_center'] = "full" # 
                ...
                for i in hnc[c3]:
                    vn = "%s_%d"%(v,i+1)
                    hnc[vn] = hnc[v][:,:,i].squeeze().expand_dims({'time':time}).astype(float)
                    hnc[vn].attrs['mesh'] = "SCHISM_hgrid"
                    dhc = hnc[vn].dims[1].split('_')[-1]
                    if dhc == 'face':
                        dhc = 'elem'
                    ...
                    if dl==2: # each variable is then 1D 
                        if  hnc[vn].attrs['data_horizontal_center'] == 'node':
                            ...
                            raise AttributeError("The data_horizontal_center option for %s is unavailable"%v)
                    else: # I don't think that all variables are on full, but it's unclear how I can make this distinction. 
                        hnc[vn].attrs['data_vertical_center'] = "half" # 
                        if  hnc[vn].attrs['data_horizontal_center'] == 'node':
                            hnc[vn].attrs['i23d'] = 3
    ...
    cs = xr.DataArray(np.nan*np.ones_like(output_nc.nSCHISM_vgrid_layers),  
                      coords = [np.zeros_like(output_nc.nSCHISM_vgrid_layers)], # has to be float here. 
                      dims = ['sigma'],name = 'Cs')
...
#                                'SED3D_dp','SED3D_rough'])
    ...
    output_nc = output_nc.drop(['q2','xl','su2','sv2']) # maybe it doesn't like variables on edge?
