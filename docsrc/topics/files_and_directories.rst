...
.. note::
  The preprocessor `prepare_schism` has a key word `prepro_output_dir` which is set to "../prepro_out" in the `main_bay_delta.yaml template <https://github.com/CADWRDeltaModeling/BayDeltaSCHISM/blob/master/templates/bay_delta/main_bay_delta.yaml>`_. You might consider changing this to the run directory but please don't use ".".
