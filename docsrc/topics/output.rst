...
In the above example, to compute flow across the "Middle River at Undine Road" station line, the upstream elements would have an index of 1, and the downstream elements would have an index of 2, so the flux across the line would be computed and output. For the "San Joaquin River near Lathrop" station line, the upstream elements would have an index of 3 and the downstream elements would have an index of 4, and so on.
