...
@click.option('--n', required=True, type=int, help="Number of lowest time steps to list.")
...
@click.argument('filename')
@click.option('--file_step', required=True, type=int, help="Time index to analyze.")
@click.option('--n', required=True, type=int, help="Number of lowest time steps to highlight.")
...
def summarize_elements(ncfile, top_n, min_count):
    ...
    filtered = [(el, appearances[el], face_x[el], face_y[el])
                for el in appearances if appearances[el] >= min_count]
...
def plot_summarized_bad_actors(bad_actors, all_x, all_y, label_top=None):
    ...
    ax.scatter(xs, ys, s=10 + 3 * counts, c=cmap(norm(counts)), label='Frequent bad actors')
    if label_top is None:
        label_top = len(counts)
    top_idx = np.argsort(-counts)[:label_top]
    for i in top_idx:
        ax.text(xs[i], ys[i], str(els[i]), fontsize=8, ha='center', va='bottom', color='black')
...
@click.command()
@click.argument('filename')
@click.option('--num_elements', required=True, type=int, help="Top (smallest dt) N elements to consider at each timestep.")
@click.option('--num_active', required=True, type=int, help="Minimum times an element must appear in top N.")
@click.option('--plot', is_flag=True, help="Plot flagged elements.")
@click.option('--label_top', type=int, default=None, help="Number of top elements to label. Default labels all.")
