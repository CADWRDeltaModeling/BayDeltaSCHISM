...
def create_arg_parser():
    ...
    parser.add_argument(
        "--start", default=None, help="model extraction start date, e.g. 2010-05-20"
    )
    parser.add_argument(
        "--model_start",
        default=None,
    ...
    parser.add_argument("--output", default=None, help="path of csv output")
    parser.add_argument(
        "--param", default=None, help="path of param.nml file to extract start date"
    )
    return parser


def find_x2(transect, thresh=2.0, convert_km=0.001, distance_bias=0.0):
    """
    ...
    index_new = (
        pd.concat((transect.index.to_series(), midpoints), axis=0)
        .sort_values()
        .iloc[0:-1]
    )
    row_new = transect.reindex(index_new).interpolate().sort_values()
    index_x2 = row_new.searchsorted(2.0)
...
def process_x2(
    ...
    dr = pd.date_range(
        start=model_start_date + pd.Timedelta(days=ts_out.index[0]),
    ...
    if ts_out.index[0].to_pydatetime() != model_extract_date:
        raise ValueError(
            f"The fort.18 file being evaluated is {ts_out.index[0].to_pydatetime()} and does not match expected extraction date {model_extract_date}"
        )
    ...
    if ncols != nroute:
        raise ValueError(
            f"Number of columns in salt output {ncols} must match number of locations in bp file {nroute}"
        )
