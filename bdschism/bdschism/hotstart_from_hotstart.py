...
def hotstart_newgrid(yaml_fn, hotstart_in, hotstart_out, in_dir, out_dir,
    ...
    # baro = params.get_baro()

    # get start date from hotstart file
    refds = xr.open_dataset(hotstart_in)
    run_start_in = params_in.run_start
    hot_date = pd.Timedelta(seconds=refds['time'].values[0]) + run_start_in # used to fill strings in temp_yaml
    timestep = params_out._namelist['CORE']['dt']['value'] # used to fill strings in temp_yaml
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as temp_file:
        ...
        fmt_string_file(yaml_fn, temp_yaml, SafeDict(**locals()), method='format_map') 
        print(f'\t Temporary yaml filename: {temp_yaml}')
    
    # get modules used from hotstart infile
    if modules is None:
        ntracers, ntrs, irange_tr, modules = sh.describe_tracers(param_nml_out) # pull module list from param_nml
